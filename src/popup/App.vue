          ...
          <div class="col">
            <div class="d-flex justify-content-between">
              ...
              <div style="display: flex; align-items: center">
                <input
                  v-bind:class="[is_waiting == '1' ? 'waiting' : 'completed']"
...
export default {
  ...
  methods: {
    get_meal: _.debounce(function () {
      var date = this.date;
      var vm = this;

      axios
        .get(`https://yammeal.com/api/meals/v2/menu/dimigo?menuDate=${date.split('-').join('')}&menuTime=전체`)
        .then(function (response) {
          ...
          // console.log('res', res);
          var meal_data = {
            아침: res['조식'] !== undefined && res['조식'].length > 0 ? res['조식'].map((x) => x['menu_name']) : ['급식 정보가 없습니다'],
            점심: res['중식'] !== undefined && res['중식'].length > 0 ? res['중식'].map((x) => x['menu_name']) : ['급식 정보가 없습니다'],
            저녁: res['석식'] !== undefined && res['석식'].length > 0 ? res['석식'].map((x) => x['menu_name']) : ['급식 정보가 없습니다'],
          };

          // alert(meal_data['점심'][0]);
